function smoothSpectrumPlot (X)
%smoothSpectrumPlot   Plots the log amplitude spectrum of a givel sound signal (x)
%                     with the smoothed spectrum overlayed which will show
%                     the formants of the sound signal.
%
%       smoothSpectrumPlot (X) takes the vowel sound and produces a plot of the
%       of the spectrum with the frequency against the log amplitude. Then
%       the smoothed spectrum is obtained and plotted over the orginal spectrum
% 
...
SmoothSpec = smooth( log10( magSegHam ), 50 );
% Plotting the smoothed spectrum on top of the original spectrum plot
plot(FreqPoints, SmoothSpec, 'r','LineWidth',2);
% Labeling the axis
xlabel('Frequency (Hz)');
